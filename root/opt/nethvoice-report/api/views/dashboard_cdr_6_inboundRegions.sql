...
DROP TABLE IF EXISTS dashboard_cdr_6_past_month;
DROP TABLE IF EXISTS dashboard_cdr_6_current_month;
DROP TABLE IF EXISTS dashboard_cdr_6_past_quarter;
DROP TABLE IF EXISTS dashboard_cdr_6_past_semester;
DROP TABLE IF EXISTS dashboard_cdr_6_past_year;
DROP TABLE IF EXISTS dashboard_cdr_6_current_year;
...
SELECT region, 
       Sum(total) AS total 
FROM   (SELECT LEFT(IF(cnum IS NULL OR cnum = "", src, cnum), 4)               AS prefix, 
              Count(*)                   AS total, 
              (SELECT regione 
              FROM   zone 
       ...
       GROUP  BY LEFT(IF(cnum IS NULL OR cnum = "", src, cnum), 4)
       UNION ALL
       SELECT LEFT(IF(cnum IS NULL OR cnum = "", src, cnum), 4)               AS prefix, 
              Count(*)                   AS total, 
              (SELECT regione 
              FROM   zone 
              WHERE  prefisso = prefix) AS region 
