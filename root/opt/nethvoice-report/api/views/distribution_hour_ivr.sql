...
CREATE TABLE distribution_hour_ivr_year_10 AS 
  SELECT Date_format(time, "%Y")                      AS period, 
         Date_format(Sec_to_time(Time_to_sec(time) - Time_to_sec(time) % 
                                                     ( 10 * 60 ) + ( 
                                 10 * 60 )), "%H:%i") AS time_10, 
         data3                                        AS ivr_id, 
         data4                                        AS ivr_name, 
         data2                                        AS choice, 
         Count(*)                                     AS tot 
...
CREATE TABLE distribution_hour_ivr_week_10 AS 
  SELECT Date_format(time, "%Y-%u")                      AS period, 
         Date_format(Sec_to_time(Time_to_sec(time) - Time_to_sec(time) % 
                                                     ( 10 * 60 ) + ( 
                                 10 * 60 )), "%H:%i") AS time_10, 
         data3                                        AS ivr_id, 
